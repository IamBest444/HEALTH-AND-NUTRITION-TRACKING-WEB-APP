...
def log_beverage_advanced(date_obj, time_obj, beverage, vol):
    ...
    log_data(FILES["water_log"], [{
        ...
        "Beverage": beverage,
        "Volume_ml": vol,
        "Effective_Hydration_ml": eff_vol
    }])
...
def load_all_databases():
    ...
    # Merge custom foods
    if not df_food.empty and not df_custom.empty:
    ...
    # Ensure string columns for safe searching
    if not df_food.empty and "Dish Name" in df_food.columns:
        df_food["Dish Name"] = df_food["Dish Name"].astype(str)

    if not df_ex.empty and "Description" in df_ex.columns:
        df_ex["Description"] = df_ex["Description"].astype(str)
...
def show_dashboard(user):
    ...
    with c3:
        st.metric("Protein", f"{stats['protein']:.0f}", f"Goal: {user['Targets']['Protein']} g")

    df_w = load_data_safe(FILES["water_log"])
    w_today = 0

    if df_w is not None:
        df_w["Date"] = pd.to_datetime(df_w["Date"], errors="coerce").dt.strftime("%Y-%m-%d")
        today = datetime.now().strftime("%Y-%m-%d")
    ...
    with c_ring:
        ...
        fig.update_layout(
            annotations=[dict(text=f"{int(net)}<br>kcal", x=0.5, y=0.5, showarrow=False, font_size=20)],
            showlegend=False,
            height=220,
...
def show_fitness(user,df_ex):
    ...
    with c1:
        ...
        if search and df_ex is not None:
            matches=df_ex[
    df_ex["Description"].astype(str).str.lower().str.contains(search.lower(), na=False)
]
